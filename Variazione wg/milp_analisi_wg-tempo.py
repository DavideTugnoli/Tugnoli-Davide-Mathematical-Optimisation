...
def execute_data_generation_code(num_iterations=10):
...
def execute_model(num_iterations=10):
    results_df = pd.DataFrame(columns=['iteration', 'wg', 'num_groups', 'num_individual_tutoring', 'execution_time'])
    try:
        for iteration in range(1, num_iterations + 1):
            # Lista dei valori wg da testare
            wg_values = [0.5, 0.6, 0.7, 0.8, 0.9, 1.0]
            for wg in wg_values:
