...
def execute_data_generation_code(num_iterations, num_students, num_mentors):
    ...
    for iteration in range(1, num_iterations + 1):
        ...
        students_df.to_csv(f'./generated_datasets/students_data_{num_students}_to_{num_mentors}_iter_{iteration}.csv', index=False)
        mentors_df.to_csv(f'./generated_datasets/mentors_data_{num_students}_to_{num_mentors}_iter_{iteration}.csv', index=False)


def execute_model(num_iterations, num_students, num_mentors):
    results_df = pd.DataFrame(columns=['iteration', 'num_groups', 'num_individual_tutoring', 'execution_time'])
    try:
        for iteration in range(1, num_iterations + 1):
                ...
                students_df = pd.read_csv(f'./generated_datasets/students_data_{num_students}_to_{num_mentors}_iter_{iteration}.csv')
                mentors_df = pd.read_csv(f'./generated_datasets/mentors_data_{num_students}_to_{num_mentors}_iter_{iteration}.csv')
                ...
                PM_j = {row['Mentor ID']: row['GPM_PM'] for index, row in mentors_df.iterrows()}
                SM_j = {row['Mentor ID']: row['GPM_SM'] for index, row in mentors_df.iterrows()}
                WM_j = {row['Mentor ID']: row['GPM_WM'] for index, row in mentors_df.iterrows()}
                ...
                for e in E:
                    ...
                    w_es[e] = (3 * SD_i[student_id] * DM_j[mentor_id] + WS_i[student_id] * SM_j[mentor_id] + (1.5 - abs(gr[student_id,subject_year] - WM_j[mentor_id])) * PM_j[mentor_id] + 2 * CY_i[student_id] + (NH_i[student_id] + help_i[student_id]))  
                ...
                num_groups = 0  # Inizializza il conteggio dei gruppi
                num_individual_tutoring = 0  # Inizializza il conteggio del tutoring individuale
                ...
                if model.status == GRB.Status.OPTIMAL:
                    # Conta il numero di gruppi e attività individuali formati
                    num_groups = sum(1 for (j, sk, t) in P if y_jkt[j, sk, t].X > 0.5)
                    num_individual_tutoring = sum(1 for e in E if y_e[e].X > 0.5)
                    ...
                    new_row = pd.DataFrame({
                        ...
                        'num_individual_tutoring': [num_individual_tutoring],
                    ...
                    # Salva i risultati in un CSV per l'analisi
                    results_df.to_csv('./generated_results/simulation_results.csv', index=False)
...
def main_analysis(num_iterations, student_to_tutor_ratios):
    results_overall = pd.DataFrame()

    for ratio in student_to_tutor_ratios:
        num_students, num_mentors = ratio

        execute_data_generation_code(num_iterations=num_iterations, num_students=num_students, num_mentors=num_mentors)

        results_df = execute_model(num_iterations=num_iterations, num_students=num_students, num_mentors=num_mentors)

        results_df['student_to_tutor_ratio'] = f"{num_students}:{num_mentors}"

        results_overall = pd.concat([results_overall, results_df], ignore_index=True)

    results_overall.to_csv('./generated_results/overall_simulation_results.csv', index=False)



if __name__ == "__main__":
    student_to_tutor_ratios = [(95, 5), (90, 10), (75, 25), (60, 40), (50, 50), (40, 60), (25, 75), (10, 90), (5, 95)]
    main_analysis(num_iterations=10, student_to_tutor_ratios=student_to_tutor_ratios)
