...
def generate_mentors(num_mentors, subjects):
    mentors_df = []
    ...
    mentors_df = pd.DataFrame(mentors_df, columns=["Mentor ID", "Subjects", "Max Group Capacity per Subject", "Social Disadvantage Preference", "Accepts Group", "Time Capacity", "Social Index", "Student Year Preference", "Social Preference", "GPM", "GPM_PM", "GPM_SM", "GPM_WM"])
    return mentors_df

def generate_students(num_students, schools, subjects_by_year, total_schools, subjects_weights):
    students_df = []
    ...
    students_df = pd.DataFrame(students_df, columns=["Student ID", "School", "Class", "Subjects", "Time Required", "Grades", "Social Disadvantage", "Accepts Group", "Home Help", "Equipment", "Year", "Not Enough Help at Home", "Weak student", "Critical Year", "Matriculation"])
    return students_df
...
def execute_data_generation_code():
    ...
    # Generazione iniziale dei tutor
    mentors_df = generate_mentors(num_mentors, subjects)
    ...
    mentors_df.to_csv('./generated_datasets/mentors_data.csv', index=False)

    for iteration in range(1, num_iterations + 1):
        students_df = generate_students(num_students, schools, subjects_by_year, total_schools, subjects_weights)
        students_df.to_csv(f'./generated_datasets/students_data_{iteration}.csv', index=False)


def execute_model(num_iterations=30):
    try:
        results_columns = ['Iterazione', 'Tutor ID', 'Capacità Massima', 'Ore Assegnate', 'Percentuale Utilizzo']
        risultati_df = pd.DataFrame(columns=results_columns)
        for iteration in range(1, num_iterations + 1):
            students_df = pd.read_csv(f'./generated_datasets/students_data_{iteration}.csv')
            mentors_df = pd.read_csv('./generated_datasets/mentors_data.csv')
            ...
            # Dizionario per raccogliere i dati dell'iterazione corrente
            # Dopo l'ottimizzazione, raccogliere i dati per ogni tutor
            for tutor_id in B:
                capacita_massima = Q[tutor_id]
                ore_assegnate = sum(x_jkt[tutor_id, sk, t].X for sk in S_bj[tutor_id] for t in range(1, 6))
                ore_assegnate += sum(x_e[e].X for e in E if e[1] == tutor_id)
                
                percentuale_utilizzo = (ore_assegnate / capacita_massima) * 100 if capacita_massima > 0 else 0
                
                # Creazione di un nuovo DataFrame per la riga da aggiungere
                nuova_riga_df = pd.DataFrame({
                    'Iterazione': [iteration], 
                    'Tutor ID': [tutor_id], 
                    'Capacità Massima': [capacita_massima], 
                    'Ore Assegnate': [ore_assegnate], 
                    'Percentuale Utilizzo': [percentuale_utilizzo]
                })

                risultati_df = pd.concat([risultati_df, nuova_riga_df], ignore_index=True)

            risultati_df.to_csv('./generated_results/risultati_utilizzo_tutor.csv', index=False)
...
def plot_tutor_utilization(csv_file_path):
    # Carica i dati dal file CSV
    data = pd.read_csv(csv_file_path)
    
    mean_utilization = data.groupby('Tutor ID')['Percentuale Utilizzo'].mean().reset_index()
    max_capacity = data[['Tutor ID', 'Capacità Massima']].drop_duplicates().set_index('Tutor ID')
    mean_utilization = mean_utilization.join(max_capacity, on='Tutor ID')
    mean_utilization = mean_utilization.sort_values('Percentuale Utilizzo', ascending=False)
    
    plt.figure(figsize=(14, 10))
    barplot = sns.barplot(x='Percentuale Utilizzo', y='Tutor ID', data=mean_utilization, orient='h')
    plt.title('Utilizzo medio della capacità dei tutor')
    plt.xlabel('Percentuale di Utilizzo (%)')
    plt.ylabel('Tutor ID')
    plt.grid(axis='x')
    plt.tight_layout()
    
    plt.xlim(0, data['Percentuale Utilizzo'].max() + 13)
    
    for index, row in mean_utilization.iterrows():
        barplot.text(102, index, f"Cap. Max: {row['Capacità Massima']}", va='center')
...
#execute_model()
plot_tutor_utilization('./generated_results/risultati_utilizzo_tutor.csv')
